...
FindLocation::saveRealPath(Request &request, map<string, Location>& locationTable, VirtualServer* server)
{
	...
	{
        if (findLocationBlock(request, uri, locationTable) == true)
		{
            setRootAlias(uri);
            ...
            if (lstat(m_path.c_str(), &d_stat) == -1)
			{
                ...
                request.m_file = m_file;
				if (request.m_method != RequestHandler::PUT || lstat(request.m_path.c_str(), &d_stat) == -1)
				{
					request.m_status = 404;
				}
            ...
            if (S_ISDIR(d_stat.st_mode) == false) {                 request.m_path = m_path.substr(0, m_path.find_last_of("/")) + "/";
				request.m_file = m_file;
                LOG(DEBUG, "1-1-1. %s", (request.m_path + request.m_file).data());
                return request.m_path + request.m_file;
            }
            else  {                 if (m_locationBlock->m_index.size() != 0 &&
						(request.m_method != RequestHandler::PUT && request.m_method != RequestHandler::POST))
                {
                    for (size_t i = 0; i < m_locationBlock->m_index.size(); i++)
                    {
            ...
            if (lstat(realPath.c_str(), &d_stat) == -1) {                 m_path = realPath.substr(0, uri.find_last_of("/")) + "/";
            ...
            if (S_ISDIR(d_stat.st_mode) == false) {                 m_path = m_root + uri.substr(0, uri.find_last_of("/")) + "/";
